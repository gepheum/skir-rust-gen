 ...
 */
function collectRustModuleSpecs(
  ...
  for (const skirModule of skirModules) {
    const rustModulePath = skirModule.path.replace(/\.skir$/, ".rs");
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (parentSpec) {
        parentSpec.childModuleNames.add(child);
        break;
      } else {
        pathToModuleSpec.set(parentPath, {
          path: parentPath,
          childModuleNames: new Set([child]),
          skirModule: null,
        });
      }
      child = parentPath;
